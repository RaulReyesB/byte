...
const compraBoleto = async (userData, personaData) => {
  const { nombre, correo1, token } = userData;
  console.log('Correo electrónico:', correo1);
  var transport = nodemailer.createTransport({
    host: process.env.EMAIL_HOST,
    port: process.env.EMAIL_PORT,
    auth: {
  ...
  try {
    await transport.sendMail({
      from: "ByteBus.com",
      to: correo1,
      subject: "Bienvenido a Byte Bus - Confirma tu compra de tu Boleto",
      text: "Gracias por elegirnos sobre todas las opciones, le aseguramos que somos la mejor opcion para usted",
      html: `
    <!DOCTYPE html>
    <html lang="es" xmlns="http://www.w3.org/1999/xhtml" xmlns:o="urn:schemas-microsoft-com:office:office">
                ...
                <tr>
                  <td
                    ...
                    <div class="col-lge"
                      ...
                      display: inline-block;
                      border-radius: 5px;">
                        <a href="http://${process.env.SERVER_HOST}:${process.env.SERVER_PORT}/viajes/confirm/${token}">
                          CONFIRMAR CUENTA
                        </a>
                      </p>
  ...
  console.log(`************Se esta intentando enviar un correo electronico al usuario: ${userData.correo1}, con el token de validacion: ${userData.token}*************`)
